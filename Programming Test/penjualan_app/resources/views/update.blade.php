...
<body>
    ...
    <form action="/edit" method="post">
        ...
        <h2>Update</h2>
        <fieldset>
            <input type="text" name="transactions_id" hidden value={{ encrypt( $transaction->id ) }}>
            <label>
                Transaction No
                <input type="number" name="no_transaction" id="no_transaction" value={{ $transaction->no_transaction }}>
            </label>
            <hr>
            <label>
                Transaction Date
                <input type="text" name="transaction_date" id="transaction_date" value={{ $transaction->transaction_date }}>
            </label>
        </fieldset>
        <fieldset>
            <h3>Detail Items</h3>
            ...
            <div class="item-container" id="item-container">
                @foreach ($transaction_details as $transaction_detail)
                    <div class="item">
                        <input name="id[]" hidden value={{ $transaction_detail->id }}>
                        <label>
                            Item Name
                            <input type="text" name="item[]" id="item" value={{ $transaction_detail->item }}>
                        </label>
                        <label>
                            Quantity
                            <input type="number" name="quantity[]" id="quantity" value={{ $transaction_detail->quantity }}>
                        </label>
                    </div>
                @endforeach
            </div>
